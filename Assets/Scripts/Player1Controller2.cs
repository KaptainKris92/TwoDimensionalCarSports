...
public class Player1Controller2 : MonoBehaviour
{
    ...
    public void VerticalFlipping() // "," key or "R1" flips the car upside down only if wheels are not touching the ground
    {
        ...
        if (!grounded && (gamepad.rightShoulder.wasPressedThisFrame | keyboard.zKey.wasPressedThisFrame))
        {
            VerticalFlip();
    ...
    public void CarJumping() // Jumping  with "/" on keyboard or "X" on controller    
    {
        ...
        if ((grounded || !doubleJump) && (gamepad.buttonSouth.wasPressedThisFrame | keyboard.cKey.wasPressedThisFrame) && facingRight)
        {
    ...
    public void Boost() //Boost with "." on keyboard or "Circle" on controller
    {
        ...
        if ((gamepad.buttonEast.wasPressedThisFrame | keyboard.xKey.wasPressedThisFrame))
        {
            GetComponent<Rigidbody2D>().AddForce(transform.right * boostForce);
        }

        if ((gamepad.buttonEast.isPressed | keyboard.xKey.isPressed))
        {
            GetComponent<Rigidbody2D>().AddForce(transform.right * boostForce);
        }
    }
    public void DoubleJumpFlipKeyboard() //Forward or backwards flip performed by pressing the jump button and holding up on the joysick (controller) or holding accelerate (keyboard) while in the air
    {
        ...
        if ((keyboard.cKey.wasPressedThisFrame | gamepad.buttonSouth.wasPressedThisFrame) && upFlipKeyboard && !grounded && facingRight && !usedFlip)
        {
        ...
        else if ((keyboard.cKey.wasPressedThisFrame | gamepad.buttonSouth.wasPressedThisFrame) && downFlipKeyboard && !grounded && facingRight && !usedFlip)
        {
            GetComponent<Rigidbody2D>().AddTorque(flipRotateForce * 100, ForceMode2D.Force);
            GetComponent<Rigidbody2D>().AddForce(-transform.right * flipForce);
            StartCoroutine(SecondWait2());
        }
        else if ((keyboard.cKey.wasPressedThisFrame | gamepad.buttonSouth.wasPressedThisFrame) && upFlipKeyboard && !grounded && !facingRight && !usedFlip)
        {
            GetComponent<Rigidbody2D>().AddTorque(flipRotateForce * 100, ForceMode2D.Force);
            GetComponent<Rigidbody2D>().AddForce(transform.right * flipForce);
            StartCoroutine(SecondWait2());
    ...
    public void UpFlipCheck() //Checks if up arrow key held + jump (slash key) pressed 
    {
        ...
        if (keyboard.tKey.isPressed | gamepad.leftStick.up.isPressed) upFlipKeyboard = true;
    ...
    public void DownFlipCheck()  //Checks if down arrow key held + jump (slash key) pressed 
    {
        ...
        if (keyboard.gKey.isPressed | gamepad.leftStick.down.isPressed) downFlipKeyboard = true;
    ...
    public void KeyboardAccelerate() // Binary acceleration 
    {
        ...
        if (grounded && (gamepad.rightTrigger.isPressed | keyboard.tKey.isPressed))
        {
    ...
    public void KeyboardDeccelerate() // Binary deceleration
    {
        ...
        if (grounded && (gamepad.leftTrigger.isPressed | keyboard.gKey.isPressed) && (vSpeed < maxSpeed))
        {
            GetComponent<Rigidbody2D>().AddForce(-transform.right * Acceleration);
        }
    }
    ...
    public void Rotate()
    {
        ...
        float rotate2;
        if (keyboard.hKey.isPressed) rotate2 = -1;
        else if (keyboard.fKey.isPressed) rotate2 = 1;
        else rotate2 = 0;
