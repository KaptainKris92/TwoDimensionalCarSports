...
public class Player2Controller : MonoBehaviour
{
    ...
    public void VerticalFlipping() // "," key or "R1" flips the car upside down only if wheels are not touching the ground
    {
        ...
        if (!grounded && (gamepad.rightShoulder.wasPressedThisFrame | keyboard.backslashKey.wasPressedThisFrame))
        {
            VerticalFlip();
    ...
    public void CarJumping() // Jumping  with "/" on keyboard or "X" on controller    
    {
        ...
        if ((grounded || !doubleJump) && (gamepad.buttonSouth.wasPressedThisFrame | keyboard.xKey.wasPressedThisFrame) && facingRight)
        {
    ...
    public void Boost() //Boost with "z" on keyboard or "Circle" on controller
    {
        var gamepad = Gamepad.current;
        var keyboard = Keyboard.current;

        if ((gamepad.buttonEast.wasPressedThisFrame | keyboard.zKey.wasPressedThisFrame))
        {
            GetComponent<Rigidbody2D>().AddForce(transform.right * boostForce);
        }

        if ((gamepad.buttonEast.isPressed | keyboard.zKey.isPressed))
        {
            GetComponent<Rigidbody2D>().AddForce(transform.right * boostForce);
        }
    }
    public void DoubleJumpFlipKeyboard() //Forward or backwards flip performed by pressing the jump button and holding up on the joysick (controller) or holding accelerate (keyboard) while in the air
    {
        ...
        if ((keyboard.xKey.wasPressedThisFrame | gamepad.buttonSouth.wasPressedThisFrame) && upFlipKeyboard && !grounded && facingRight && !usedFlip)
        {
        ...
        else if ((keyboard.xKey.wasPressedThisFrame | gamepad.buttonSouth.wasPressedThisFrame) && downFlipKeyboard && !grounded && facingRight && !usedFlip)
        {
            GetComponent<Rigidbody2D>().AddTorque(flipRotateForce * 100, ForceMode2D.Force);
            GetComponent<Rigidbody2D>().AddForce(-transform.right * flipForce);
            StartCoroutine(SecondWait2());
        }
        else if ((keyboard.xKey.wasPressedThisFrame | gamepad.buttonSouth.wasPressedThisFrame) && upFlipKeyboard && !grounded && !facingRight && !usedFlip)
        {
            GetComponent<Rigidbody2D>().AddTorque(flipRotateForce * 100, ForceMode2D.Force);
            GetComponent<Rigidbody2D>().AddForce(transform.right * flipForce);
            StartCoroutine(SecondWait2());
    ...
    public void UpFlipCheck() //Checks if up arrow key held + jump (slash key) pressed 
    {
        ...
        if (keyboard.yKey.isPressed | gamepad.leftStick.up.isPressed) upFlipKeyboard = true;
    ...
    public void DownFlipCheck()  //Checks if down arrow key held + jump (slash key) pressed 
    {
        ...
        if (keyboard.hKey.isPressed | gamepad.leftStick.down.isPressed) downFlipKeyboard = true;
    ...
    public void KeyboardAccelerate() // Binary acceleration 
    {
        ...
        if (grounded && (gamepad.rightTrigger.isPressed | keyboard.yKey.isPressed))
        {
    ...
    public void KeyboardDeccelerate() // Binary deceleration
    {
        ...
        if (grounded && (gamepad.leftTrigger.isPressed | keyboard.hKey.isPressed) && (vSpeed < maxSpeed))
        {
            GetComponent<Rigidbody2D>().AddForce(-transform.right * Acceleration);
        }
    }
    ...
    public void Rotate()
    {
        ...
        float rotate2;
        if (keyboard.jKey.isPressed) rotate2 = -1;
        else if (keyboard.gKey.isPressed) rotate2 = 1;
        else rotate2 = 0;
